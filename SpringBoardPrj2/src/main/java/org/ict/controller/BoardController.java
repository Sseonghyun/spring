...
public class BoardController {
	...
	@GetMapping("/register") // get방식으로만 접속되는 /board/register
	...
	// service에서 get()을 호출해 가져온 글 하나의 정보를 get.jsp로 보내줍니다.
	...
	// 마음대로 글 삭제를 하는 경우가 생길 수 있으므로
	// 무조건 삭제 버튼을 클릭해서 삭제할 수 있도록
	// post방식 접근만 허용
	// bno를 받아서 삭제하고, 삭제 후에는 "success"라는 문자열을 .jsp로 보내줍니다.
	// 삭제가 완료되면 redirect 기능을 이용해 list페이지로 넘어가게
	// 코드 및 파라미터를 내부에 작성해주세요.
	@PostMapping("/remove")
	public String remove(Long bno,  RedirectAttributes rttr) {
		log.info("삭제로직 : " + bno);
		service.remove(bno); // 글삭제
